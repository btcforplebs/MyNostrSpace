...
const FriendsPage = () => {
    ...
    const { friends: friendPubkeys, loading: friendsLoading, fetchProfiles } = useFriends(hexPubkey || undefined);
            ...
            <div style={{ padding: 20 }}>
                ...
                {totalPages > 1 && (
                    <div className="pagination" style={{ margin: '10px 0', textAlign: 'right' }}>
                        ...
                        {' '}
                        <button disabled={currentPage === 0} onClick={() => setCurrentPage(p => p - 1)}>&laquo; Prev</button>
                        {' '}
                        <button disabled={currentPage >= totalPages - 1} onClick={() => setCurrentPage(p => p + 1)}>Next &raquo;</button>
                    </div>
                )}
                ...
                {pageLoading ? (
                    ...
                    <div className="friends-grid">
                        {currentProfiles.map(friend => (
                            <div key={friend.pubkey} className="friend-card" style={{
                                width: 100,
                                textAlign: 'center',
                                marginBottom: 20,
                                fontSize: '10pt'
                                ...
                                <div style={{ marginBottom: 5 }}>
                                    <Link to={`/p/${friend.profile?.nip05 || friend.profile?.name || friend.pubkey}`} style={{ fontWeight: 'bold', textDecoration: 'none' }}>
