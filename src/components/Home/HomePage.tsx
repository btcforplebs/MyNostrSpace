...
export const HomePage: React.FC = () => {
    ...
    const MOODS = ["None", "Happy", "Sad", "Angry", "Confused", "Excited", "Sleepy", "Bored", "Hyper", "Creative"];
    ...
    const fetchHomeData = useCallback(async () => {
        ...
        try {
            ...
            await Promise.all(sortedNotifications.map(async (e) => {
                try {
            ...
            const parentIds = replyEvents.map(n => n.tags.find(t => t[0] === 'e' && (t[3] === 'reply' || !t[3]))?.[1]).filter(Boolean) as string[];
            ...
            <div className="home-content">
                ...
                <div className="home-header-sub">
                    <div className="my-url-text">
                        My URL: <Link to={`/p/${user.pubkey}`}>mynostrspace.com/{user.pubkey.slice(0, 8)}</Link> [ <Link to="/edit-profile">Edit Profile</Link> ]
                    </div>
                    <div>{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric' })}</div>
                </div>
                ...
                <div className="home-layout">
                    ...
                    <div className="home-column-left">
                        <div className="home-box user-pic-box">
                            <img src={user.profile?.picture || "https://via.placeholder.com/170"} alt="Profile" className="user-pic" />
                            <ul className="profile-stats">
                                <li><strong>Profile Views:</strong> 1,102</li>
                                <li><strong>Last Login:</strong> {new Date().toLocaleDateString()}</li>
                            </ul>
                        </div>

                        <div className="home-box">
                            <ul className="quick-links" style={{ padding: '5px' }}>
                                <li><strong>Photos:</strong> <a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Edit</a> | <a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Upload</a></li>
                                <li><strong>Videos:</strong> <a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Edit</a> | <a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Upload</a></li>
                                <li><a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Manage Calendar</a></li>
                                <li><a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Manage Blog</a></li>
                                <li><a href="#" onClick={(e) => { e.preventDefault(); alert("Coming soon!"); }}>Manage Reviews</a></li>
                            </ul>
                        </div>
                                    ...
                                    <>
                                            ...
                                            {(showAllNotifications ? notifications : notifications.slice(0, 5)).map(n => {
                                                ...
                                                return (
                                                    <li key={n.id} className="notification-item">
                                                        <img src={n.author.profile?.picture || "https://via.placeholder.com/20"} className="notification-user-pic" alt="" />
                                                        <div className="notification-content">
                                                            <Link to={`/p/${n.pubkey}`} className="notification-user-name">
                                                                {n.author.profile?.name || n.pubkey.slice(0, 8)}
                                                            </Link>
                                                            <span className="notification-action"> <Link to={`/thread/${n.id}`} style={{ color: '#333', textDecoration: 'none' }}>{actionText}</Link></span>

                                                            {isComment && (
                                                                <div className="notification-parent-snippet">
                                                                    {(() => {
                                                                        const parentId = n.tags.find(t => t[0] === 'e' && (t[3] === 'reply' || !t[3]))?.[1];
                                                                        const parent = parentId ? parentEvents[parentId] : null;
                                                                        if (parent) {
                                                                            const content = parent.content.replace(/^Mood: (.*?)\n\n/, '');
                                                            ...
                                                            <div className="notification-time">
                                                                {n.created_at ? new Date(n.created_at * 1000).toLocaleDateString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                                                            </div>
                                                        </div>
                                                    </li>
                                                );
                                        ...
                                        {!showAllNotifications && notifications.length > 5 && (
                                            <div style={{ textAlign: 'right', marginTop: '5px' }}>
                                                <span
                                                    style={{ color: '#003399', fontSize: '8pt', cursor: 'pointer', fontWeight: 'bold' }}
                    ...
                    <div className="home-column-middle">
                        <div className="home-box status-mood-box">
                            <div className="status-mood-header">
                                Status and Mood
                            </div>
                            <div className="status-input-container">
                                ...
                                <div className="status-controls">
                                    ...
                                    <button className="post-status-btn" onClick={handlePostStatus}>Post</button>
                                </div>
                            </div>
                            <div className="feed-container">
                                ...
                                {!loading && feed.length === 0 && <div style={{ padding: '10px' }}>No updates from friends yet.</div>}
                            </div>
                        </div>
                    </div>
