        ...
        <div className="wavlake-player">
            <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span className="powered-by-wavlake" style={{ fontSize: '7pt', fontWeight: 'normal' }}>
                    powered by <a href="https://wavlake.com" target="_blank" rel="noreferrer" style={{ color: '#003399', textDecoration: 'none', fontWeight: 'bold' }}>wavlake</a>
                </span>
            </div>

            {/* Player Area */}
            <div className="retro-player-box" style={{ background: '#f5f5f5', border: '1px solid #ccc', padding: '10px', textAlign: 'center' }}>
                <div style={{ background: '#000', padding: '5px', borderRadius: '12px' }}>
                    {isLegacyEmbed ? (
                        <iframe
                            key={playbackUrl}
                            style={{ borderRadius: '12px' }}
                            src={playbackUrl && !playbackUrl.includes('autoplay=true')
                        ...
                        <div style={{ color: 'white', padding: '20px' }}>
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                ...
                                {currentTrackData?.link && (
                                    <a href={currentTrackData.link} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', color: '#aaa', textDecoration: 'none', fontSize: '8pt' }}>
                                        <img src={wavlakeLogo} alt="Wavlake" style={{ width: '12px', height: '12px', marginRight: '4px' }} />
                                        View on Wavlake
                                    </a>
                                )}
                            </div>
                ...
                {activePlaylist && activePlaylist.length > 1 && (
                    <div className="playlist-controls" style={{ marginTop: '10px', display: 'flex', justifyContent: 'center', gap: '10px' }}>
                        <button
                            onClick={() => setCurrentIndex(prev => prev === 0 ? activePlaylist.length - 1 : prev - 1)}
                                ...
                                <tr key={i} style={{ backgroundColor: i % 2 === 0 ? '#ffcc99' : '#ffffff' }}>
                                    <td style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            <span onClick={() => setCurrentIndex(i)} style={{ cursor: 'pointer', fontWeight: currentIndex === i ? 'bold' : 'normal', color: currentIndex === i ? 'red' : 'blue', textDecoration: 'underline' }}>
                                                {track.title}
                                            </span>
                                            {track.link && (
                                                <a href={track.link} target="_blank" rel="noreferrer" style={{ fontSize: '7pt', color: '#666', textDecoration: 'none', marginLeft: '5px' }}>
                                                    [view on wavlake]
                                                </a>
                                            )}
                                        </div>
                                        {currentIndex === i && <span style={{ color: 'red', fontSize: '8px' }}>♫ Playing</span>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
