...
export const WavlakeSearch = ({ onSelect }: WavlakeSearchProps) => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const res = await fetch(`https://wavlake.com/api/v1/content/search?term=${encodeURIComponent(query)}`);
    ...
    return (
        <div className="wavlake-search">
            ...
            <div className="results-list" style={{ marginTop: '10px', maxHeight: '200px', overflowY: 'auto', border: '1px solid #ccc' }}>
                {results.map(track => (
                    <div
                        key={track.id}
