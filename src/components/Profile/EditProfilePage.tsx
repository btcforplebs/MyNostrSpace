...
const EditProfilePage = () => {
  ...
  const {
    ...
    loading: extLoading,
    publish: publishExt,
  } = useExtendedProfile(user?.pubkey);
  const {
    profile: basicProfile,
    loading: basicLoading,
    publishProfile: publishBasic,
  ...
  useEffect(() => {
    if (basicProfile) {
      ...
      setBasicFormData((prev) => {
        // Only update if it's actually different to avoid unnecessary cycles
        if (JSON.stringify(prev) === JSON.stringify(basicProfile)) return prev;
        return basicProfile;
      });
    }
  }, [basicProfile]);
  ...
  const handleSave = async () => {
    await Promise.all([publishBasic(basicFormData), publishExt(extFormData)]);
    ...
    <div className="edit-profile-container">
      ...
      <div className="edit-header">
        ...
        <div className="edit-nav">
          ...
          <button onClick={() => navigate('/edit-layout')} style={{ background: '#cce5ff' }}>
            Customize Design
          </button>
              ...
              <>
                {musicList.length > 0 ? (
                  <div style={{ marginBottom: '10px' }}>
                    {musicList.map(
                      (track: { title: string; url: string; link?: string }, i: number) => (
                        <div
                          key={i}
                          style={{
                            padding: '8px',
                ...
                <WavlakeSearch
                  onSelect={(track: { title: string; url: string; link?: string }) => {
