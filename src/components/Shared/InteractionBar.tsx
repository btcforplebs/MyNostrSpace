...
export const InteractionBar: React.FC<InteractionBarProps> = ({ event, onCommentClick }) => {
    ...
    return (
        <div className="interaction-bar" style={{
            ...
            <a href="#" onClick={handleLike} style={{ color: isLiked ? '#f04e30' : '#003399', fontWeight: isLiked ? 'bold' : 'normal' }}>
                {isLiked ? '♥ Liked' : 'like'} ({likes})
            </a>
            |
            <a href="#" onClick={(e) => { e.preventDefault(); onCommentClick?.(); }} style={{ color: '#003399' }}>
                comment ({comments})
            </a>
            |
            ...
            {zapInvoice && (
                <div className="zap-modal" style={{
                    ...
                    <h3 style={{ margin: '0 0 10px 0', background: '#ff9933', color: 'black', padding: '5px' }}>Scan to Zap</h3>
                    <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${zapInvoice}`}
                        alt="Zap QR Code"
                        style={{ border: '1px solid #ccc', marginBottom: '10px' }}
                    />
                    <div style={{ fontSize: '8pt', marginBottom: '15px', wordBreak: 'break-all', maxHeight: '100px', overflowY: 'auto', border: '1px solid #ccc', padding: '5px', textAlign: 'left' }}>
                        <code>{zapInvoice}</code>
                    </div>
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                        <button
                            onClick={() => { navigator.clipboard.writeText(zapInvoice); alert("Invoice copied!"); }}
                            style={{ padding: '5px 10px', cursor: 'pointer', fontWeight: 'bold', border: '1px solid #ccc' }}
                        >
                            Copy Invoice
                        </button>
                        <button
                            onClick={() => setZapInvoice(null)}
                            style={{ padding: '5px 10px', cursor: 'pointer', fontWeight: 'bold', border: '1px solid #ccc', background: '#ccc' }}
                        >
                            Close
                        </button>
                    </div>
