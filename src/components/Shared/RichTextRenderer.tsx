...
export const RichTextRenderer: React.FC<RichTextRendererProps> = ({ content, style, className }) => {
    ...
    return (
        <div style={style} className={className}>
            {lines.map((line, lineIndex) => (
                <div key={lineIndex} style={{ minHeight: '1em', wordWrap: 'break-word', overflowWrap: 'break-word', wordBreak: 'break-word' }}>
                    {line.split(' ').map((word, wordIndex) => {
                        ...
                        if (isUrl) {
                            ...
                            if (lowerUrl.match(/\.(jpg|jpeg|png|gif|webp)(\?.*)?$/)) {
                                return (
                                    <div key={wordIndex} style={{ marginTop: '5px', marginBottom: '5px', display: 'block' }}>
                                        <img
                                            src={url}
                        ...
                        if (isNostrMatch) {
                            ...
                            try {
                                ...
                                if (type === 'npub' || type === 'nprofile') {
                                    ...
                                    return (
                                        <Link key={wordIndex} to={`/p/${pubkey}`} style={{ color: '#003399', fontWeight: 'bold' }}>
                                            {word}
                                        </Link>
                                    );
                                }
